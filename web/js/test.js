QUnit.test("dueDate", function(assert) {
  ...
  var assertDueDate = function(start, hours, end, msg, verbose) {
    var dueDate = dueDateCalculator(start, hours),
        message = verbose ? msg + " " + start.toString() + " vs " + end.toString() : msg;
  ...
  assertDueDate(new Date(2014, 11, 22, 15, 0, 0, 0), 2, new Date(2014, 11, 23, 9), "Adding some time that ends the next day", true);
});
